...
//  signal (SIGCONT, SignalHandler);  // do this intrinsically after return from SIGTSTP
    ...
    cycle = DEFAULT_TIME;  // get tick count 
    if (cycle <= 0) cycle = 1;
    ...
    for (i = 0; i < cycle;) {          // tick 
        ...
        if (signal_SIGCONT) {
            ...
            fflush(output);
            WriteAnswer(ans_file, nick_name, "SIGCONT");
        }
        ...
        if (rc == 0 || (stoptick-starttick) > clktck/2) {
            ...
            WriteAnswer(ans_file, nick_name, "tick");
        }
                
        if (signal_SIGINT) {
            fprintf(output,"%s%7d; SIGINT" BLACK NORMAL "\n", colour, (int) pid);
            WriteAnswer(ans_file, nick_name, "SIGINT");
            exit(0);
        }
        if (signal_SIGQUIT) {
            fprintf(output,"%s%7d; SIGQUIT" BLACK NORMAL "\n", colour, (int) pid);
            WriteAnswer(ans_file, nick_name, "SIGQUIT");
            exit(0);
        }
        if (signal_SIGHUP) {
            fprintf(output,"%s%7d; SIGHUP" BLACK NORMAL "\n", colour, (int) pid);
            WriteAnswer(ans_file, nick_name, "SIGHUP");
            exit(0);
        }
        if (signal_SIGTSTP) {
            ...
            fflush(output);
            WriteAnswer(ans_file, nick_name, "SIGTSTP");
        ...
        if (signal_SIGABRT) {
            ...
            fflush(output);
            WriteAnswer(ans_file, nick_name, "SIGABRT");
            signal (SIGABRT, SIG_DFL);
            raise (SIGABRT);
        }
        if (signal_SIGTERM) {
            fprintf(output,"%s%7d; SIGTERM" BLACK NORMAL "\n", colour, (int) pid);
            WriteAnswer(ans_file, nick_name, "SIGTERM");
            exit(0);
        }                
        fflush(output);
    }
...
void WriteAnswer(char * nick_name, char * content);

Write the scheduling answer into auxiliary file.
*******************************************************************/

void WriteAnswer(char * ans_file, char * nick_name, char * content)
{
    FILE * ans_aux_file = fopen(ans_file, "a+");
    fprintf(ans_aux_file, "%s %s\n", nick_name, content);
    fflush(ans_aux_file);
    fclose(ans_aux_file);
}
